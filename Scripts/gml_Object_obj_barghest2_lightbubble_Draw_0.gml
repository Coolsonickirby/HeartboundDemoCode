if (global.darkening_active == 1)
{
    if (surface_exists(global.shadow_surface))
    {
        ...
        for (var xx = light_bubbles; xx > 0; xx--)
        {
            if (players_exist == 1)
            {
                with (obj_player_parent)
                {
                    ...
                    draw_ellipse(range_left, range_top, range_right, range_bottom, false);
                    draw_set_alpha(obj_barghest2_lightbubble.shadow_level * (light_base_shadow + (light_step_shadow * xx)));
        ...
        if (instance_exists(obj_save_darkworld))
        {
            with (obj_save_darkworld)
            {
                gpu_set_blendmode(bm_subtract);
                draw_sprite_ext(sprite_base, image_index, round(x), round(y), image_xscale, image_yscale, image_angle, c_black, 1);
                draw_sprite_ext(sprite_index, image_index, round(x), round(y) - 25, image_xscale, image_yscale, image_angle, c_black, 1);
                gpu_set_blendmode(bm_normal);
            }
        }
        ...
        if (followers_exist == 1)
        {
            with (obj_follower_parent)
            {
                ...
                draw_sprite_ext(sprite_index, image_index, round(x), round(y), image_xscale, image_yscale, image_angle, c_black, 1);
                gpu_set_blendmode(bm_normal);
                draw_sprite_ext(sprite_index, image_index, round(x), round(y), image_xscale, image_yscale, image_angle, c_black, obj_barghest2_lightbubble.shadow_level * light_base_shadow);
            }
        }
